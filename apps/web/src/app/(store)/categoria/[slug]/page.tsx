...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!category) {
    return { title: `Categoria não encontrada | ${siteConfig.name}`, robots: 'noindex' }
  }
    ...
    <>
      ...
      <BreadcrumbJsonLd
        items={[
          ...
          { name: category.name, url: `https://${siteConfig.domain}/categoria/${category.slug}` },
        ]}
      />
      ...
      {products.length === 0 ? (
        <EmptyState
          ...
          action={{ label: 'Ver catálogo completo', href: '/' }}
        />
      ) : (
        <>
