...
function signAccessToken(user: UserWithAuth): string {
  // Payload inclui sub and role only.
  // The authenticate hook ALWAYS fetches role from the database —
  // the token values are informational, not authoritative.
  return jwt.sign(
...
/**
 * Autentica usuário por email + password within the default tenant.
 */
export async function login(email: string, password: string, siteId: string = 'colibri'): Promise<TokenPair> {
